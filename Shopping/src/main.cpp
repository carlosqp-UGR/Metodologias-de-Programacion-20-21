...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    string rows [1] = {"pr"};
    string labels[M.columns()];
    M.getColLabels(labels);
    Matrix antPR(rows, 1, labels, M.columns(), 1.0/M.columns());
    
    // Iterate until the max number of iteration (MAX_ITER) is reached or the convergency value is less than the threshold
    Matrix newPR;
    double value=0, nConvergenceValues[n];
    int n_iterations = 0;
    bool stable=false;
    
    while (n_iterations<max_iter && !stable) {
        newPR = antPR*M;
        // Calculate value (initially 0)
        value = 0;
        for (int i=0; i<newPR.columns(); i++) {
            value += fabs(newPR(0,i)-antPR(0,i));
        }
        
        if(n_iterations<n) {
            nConvergenceValues[n_iterations]=value;
            cout << endl << "PageRank at iteration " << n_iterations << ":" << endl;
            cout << newPR;
        }
    ...
    if (stable) {
        cout << endl << "Convergence reached in " << n_iterations << " iterations (stability last value = " << value << ")" << endl;
    } else {
        cout << endl << "Convergence unreached in " << max_iter << " iterations (stability last value = " << value << ")" << endl;
    }
    
    // Show n stability values
    cout << endl << "First " << n << " stability values" << endl;
    for (int i=0; i<n; i++)
        cout << nConvergenceValues[i] << "  ";
    cout << endl;
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    for (ub; ub<idx.size();) {
        ...
        for (int i=lb; i<ub; i++) {
            brandA = evs.at(idx.at(i).getPos()).getBrand();
            if (brandA != "") {
                for (int j=i+1; j<ub; j++) {
                    brandB = evs.at(idx.at(j).getPos()).getBrand();
                    if (brandB != "" && brandA != brandB) {
                        m_adj(brandA, brandB) += 1;
                        m_adj(brandB, brandA) += 1;
                    }
                }
            }
        }
...
void shopping5(int argc, char **argv ){
    ...
    cout << endl << "Number of events with non null session in index: " << idxSesion.size() << endl;
    ...
    cout << endl << "There are " << n_values << " unique brands in the filtered sessions" << endl;
